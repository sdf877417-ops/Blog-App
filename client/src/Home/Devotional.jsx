...
function Devotional() {
  const { blogs } = useAuth();
  // const devotinalBlogs = blogs.filter();
  const devotionalBlogs = blogs.filter((elem) => elem.category == "devotionalBlogs");
    ...
    <div className="w-max-screen">
      {devotionalBlogs && devotionalBlogs.length > 0 ? (
        ...
        <h1 className="text-center text-2xl font-bold mt-10">
          No devotionalBlogs Blogs Found
        </h1>
      )}
    </div>
  );
